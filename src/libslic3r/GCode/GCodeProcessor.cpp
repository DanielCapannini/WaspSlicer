...
static const float DEFAULT_ACCELERATION = 1500.0f; // Wasp Firmware 1_75mm_MK2
static const float DEFAULT_RETRACT_ACCELERATION = 1500.0f; // Wasp Firmware 1_75mm_MK2
...
// taken from WaspResearch.ini - [printer:Original Wasp i3 MK2.5 MMU2]
...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    { EProducer::WaspSlicer, "generated by WaspSlicer" },
    { EProducer::Slic3rPE,    "generated by Slic3r Wasp Edition" },
...
void GCodeProcessor::apply_config(const PrintConfig& config)
{
    ...
    m_producer = EProducer::WaspSlicer;
    ...
    // With MM setups like Wasp MMU2, the filaments may be expected to be parked at the beginning.
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by WaspSlicer,
        // extract the config from it
        if (m_producer == EProducer::WaspSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from WaspSlicer's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::WaspSlicer) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::WaspSlicer: { return process_waspslicer_tags(comment); }
...
bool GCodeProcessor::process_waspslicer_tags(const std::string_view comment)
{
    return false;
}
...
void GCodeProcessor::process_G2_G3(const GCodeReader::GCodeLine& line, bool clockwise)
{
    ...
    // reference:
    // Wasp-Firmware\Firmware\motion_control.cpp - mc_arc()
    // https://github.com/wasp3d/Wasp-Firmware/blob/MK3/Firmware/motion_control.cpp
...
void GCodeProcessor::process_G92(const GCodeReader::GCodeLine& line)
{
    ...
    if (!any_found && !line.has_unknown_axis()) {
        // The G92 may be called for axes that WaspSlicer does not recognize, for example see GH issue #3510, 
...
void GCodeProcessor::process_M108(const GCodeReader::GCodeLine& line)
{
    ...
    // by the analyzer - see https://github.com/wasp3d/WaspSlicer/issues/2566
...
void GCodeProcessor::process_M132(const GCodeReader::GCodeLine& line)
{
    ...
    // Using this command to reset the axis origin to zero helps in fixing: https://github.com/wasp3d/WaspSlicer/issues/3082
    ...
    for (size_t i = 0; i < static_cast<size_t>(PrintEstimatedStatistics::ETimeMode::Count); ++i) {
        if (static_cast<PrintEstimatedStatistics::ETimeMode>(i) == PrintEstimatedStatistics::ETimeMode::Normal ||
            ...
            if (line.has_value('S', value)) {
                ...
                // It is also generated by WaspSlicer to control acceleration per extrusion type
...
void GCodeProcessor::process_T(const std::string_view command)
{
    if (command.length() > 1) {
        ...
        if (! parse_number(command.substr(1), eid) || eid < 0 || eid > 255) {
            // Specific to the MMU2 V2 (see https://www.help.wasp3d.com/en/article/wasp-specific-g-codes_112173):
            if ((m_flavor == gcfMarlinLegacy || m_flavor == gcfMarlinFirmware) && (command == "Tx" || command == "Tc" || command == "T?"))
                return;

            // T-1 is a valid gcode line for RepRap Firmwares (used to deselects all tools) see https://github.com/wasp3d/WaspSlicer/issues/5677
            ...
            if (m_extruder_id != id) {
                if (((m_producer == EProducer::WaspSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) && id >= m_result.extruders_count) ||
                    ((m_producer != EProducer::WaspSlicer && m_producer != EProducer::Slic3rPE && m_producer != EProducer::Slic3r) && id >= m_result.extruder_colors.size()))
