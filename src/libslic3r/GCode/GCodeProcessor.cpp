...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::WaspSlicer: { return process_prusaslicer_tags(comment); }
...
void GCodeProcessor::process_M108(const GCodeReader::GCodeLine& line)
{
    ...
    // by the analyzer - see https://github.com/prusa3d/WaspSlicer/issues/2566
...
void GCodeProcessor::process_M132(const GCodeReader::GCodeLine& line)
{
    ...
    // Using this command to reset the axis origin to zero helps in fixing: https://github.com/prusa3d/WaspSlicer/issues/3082
...
void GCodeProcessor::process_T(const std::string_view command)
{
    if (command.length() > 1) {
        ...
        if (! parse_number(command.substr(1), eid) || eid < 0 || eid > 255) {
            ...
            // T-1 is a valid gcode line for RepRap Firmwares (used to deselects all tools) see https://github.com/prusa3d/WaspSlicer/issues/5677
