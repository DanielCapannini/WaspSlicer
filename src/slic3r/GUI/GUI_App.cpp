...
static void generic_exception_handle()
{
    ...
    try {
        ...
        wxString errmsg = _L("WaspSlicer has encountered a localization error. "
                             "Please report to WaspSlicer team, what language was active and in which scenario "
...
{
	//app config initializes early becasuse it is used in instance checking in WaspSlicer.cpp
...
// gets path to WaspSlicer.ini, returns semver from first line comment
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("WaspSlicer ");
...
{
	// Profiles for the alpha are stored into the WaspSlicer-alpha directory to not mix with the current release.
        ...
        if (!error.empty()) {
            ...
            if (is_editor()) {
                throw Slic3r::RuntimeError(
                    _u8L("Error parsing WaspSlicer config file, it is probably corrupted. "
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
    ...
    if (app_config->get_bool("show_splash_screen")) {
        ...
        scrn = new SplashScreen(bmp.IsOk() ? bmp : get_bmp_bundle("WaspSlicer", 400)->GetPreferredBitmapSizeAtScale(1.0), 
...
//     update_mode(); // !!! do that later
        ...
        boost::starts_with(crash_reason,"crashed"))
    {
        ...
        InfoDialog dialog(nullptr,
            _L("WaspSlicer started after a crash"),
            format_wxstr(_L("WaspSlicer crashed last time when attempting to set window position.\n"
...
void GUI_App::init_ui_colours()
{
    ...
    m_color_hovered_btn_label       = is_dark_mode ? wxColour(40, 51, 253) : wxColour(1, 22, 255);
    m_color_default_btn_label       = is_dark_mode ? wxColour(100, 144, 255): wxColour(0, 61, 203);
    m_color_selected_btn_bg         = is_dark_mode ? wxColour(62, 72, 95)   : wxColour(216, 220, 228);
#else
...
void GUI_App::check_printer_presets()
{
    ...
    for (const std::string& preset_name : preset_names)
        msg_text += "\n    \"" + from_u8(preset_name) + "\",";
    msg_text.RemoveLast();
    msg_text += "\n\n" + _L("But since this version of WaspSlicer we don't show this information in Printer Settings anymore.\n"
...
void GUI_App::import_model(wxWindow *parent, wxArrayString& input_files) const
{
    input_files.Clear();
    wxFileDialog dialog(parent ? parent : GetTopWindow(),
        _L("Choose one or more files (STL/3MF/STEP/OBJ/AMF/WASP):"),
...
bool GUI_App::load_language(wxString language, bool initial)
{
    if (initial) {
    	...
    	// Get the active language from WaspSlicer.ini, or empty string if the key does not exist.
        language = app_config->get("translation_language");
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(trace) << boost::format("translation_language provided by WaspSlicer.ini: %1%") % language;
		...
		}
        {
            ...
            // For example in our case it's trying to load "en_CZ" and as a result WaspSlicer catch warning message.
	    	...
	    	// Let the wxFileTranslationsLoader enumerate all translation dictionaries for WaspSlicer
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by WaspSlicer: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}

    if (language_info == nullptr) {
        // WaspSlicer does not support the Right to Left languages yet.
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
    	...
    	wxString message = "Switching WaspSlicer to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "WaspSlicer - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening WaspSlicer by drag & dropping a G-Code onto WaspSlicer icon in Finder,
...
bool GUI_App::open_browser_with_warning_dialog(const wxString& url, wxWindow* parent/* = nullptr*/, bool force_remember_choice /*= true*/, int flags/* = 0*/)
{
    ...
    if (force_remember_choice || app_config->get(option_key).empty()) {
        if (app_config->get(option_key).empty()) {
            RichMessageDialog dialog(parent, _L("Open hyperlink in default browser?"), _L("WaspSlicer: Open hyperlink"), wxICON_QUESTION | wxYES_NO);
            ...
            if (dialog.IsCheckBoxChecked()) {
                ...
                wxString msg =
                    _L("WaspSlicer will remember your choice.") + "\n\n" +
                    _L("You will not be asked about it again on hyperlinks hovering.") + "\n\n" +
                    format_wxstr(_L("Visit \"Preferences\" and check \"%1%\"\nto changes your choice."), preferences_item);

                MessageDialog msg_dlg(parent, msg, _L("WaspSlicer: Don't ask me again"), wxOK | wxCANCEL | wxICON_INFORMATION);
    ...
    // and will be shown only when "Suppress to open hyperlink in browser" is ON.
        ...
        MessageDialog dialog(parent, _L("Open hyperlink in default browser?"), _L("WaspSlicer: Open hyperlink"), wxICON_QUESTION | wxYES_NO);
