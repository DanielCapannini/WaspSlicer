...
namespace instance_check_internal
{
	...
	static HWND l_wasp_slicer_hwnd;
	static BOOL CALLBACK EnumWindowsProc(_In_ HWND   hwnd, _In_ LPARAM lParam)
	{
		//checks for other instances of waspslicer, if found brings it to front and return false to stop enumeration and quit this instance
		...
		if (wndTextString.find(L"WaspSlicer") != std::wstring::npos && classNameString == L"wxWindowNR") {
	...
	static bool send_message(const std::string& message, const std::string &version)
	{
		if (!EnumWindows(EnumWindowsProc, 0)) {
			...
			SendMessage(l_wasp_slicer_hwnd, WM_COPYDATA, 0, (LPARAM)&data_to_send);
			return true;  
		}
	    return false;
	}
	...
	static bool  send_message(const std::string &message_text, const std::string &version)
	{
		...
		if ( !checker->IsAnotherRunning() ) */
		{
			...
			//std::string		interface_name = "com.wasp3d.waspslicer.InstanceCheck";
			std::string		interface_name = "com.wasp3d.waspslicer.InstanceCheck.Object" + version;
			std::string   	method_name = "AnotherInstance";
			//std::string		object_name = "/com/wasp3d/waspslicer/InstanceCheck";
			std::string		object_name = "/com/wasp3d/waspslicer/InstanceCheck/Object" + version;
...
#ifdef _WIN32
		else if (it->rfind("waspslicer://open/?file=", 0) == 0)
#else
	    else if (it->rfind("waspslicer://open?file=", 0) == 0)
#endif
...
namespace MessageHandlerDBusInternal
{
	...
	static void respond_to_introspect(DBusConnection *connection, DBusMessage *request) 
	{
	    ...
	    const char  *introspection_data =
	        ...
	        "     </method>"
	        "   </interface>"
	        "   <interface name=\"com.wasp3d.waspslicer.InstanceCheck\">"
	...
	//method AnotherInstance receives message from another WaspSlicer instance 
	...
	static DBusHandlerResult handle_dbus_object_message(DBusConnection *connection, DBusMessage *message, void *user_data)
	{
	    ...
	    std::string our_interface  = "com.wasp3d.waspslicer.InstanceCheck.Object" + wxGetApp().get_instance_hash_string();
...
void OtherInstanceMessageHandler::listen()
{
	...
	std::string			 interface_name = "com.wasp3d.waspslicer.InstanceCheck.Object" + instance_hash;
    std::string			 object_name 	= "/com/wasp3d/waspslicer/InstanceCheck/Object" + instance_hash;
	...
	if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != name_req_val) {
		BOOST_LOG_TRIVIAL(error) << "Not primary owner of DBus name - probably another WaspSlicer instance is running.";
	    BOOST_LOG_TRIVIAL(error) << "Dbus Messages listening terminating.";
	    dbus_connection_unref(conn);
	    return;
	}
