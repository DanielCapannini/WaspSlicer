    ...
    if (!m_gl_initialized) {
...
#if ENABLE_OPENGL_ES
                _utf8(L("WaspSlicer requires OpenGL ES 2.0 capable graphics driver to run correctly, \n"
...
#elif ENABLE_GL_CORE_PROFILE
                _utf8(L("WaspSlicer requires OpenGL %s capable graphics driver to run correctly, \n"
...
#else
                _utf8(L("WaspSlicer requires OpenGL 2.0 capable graphics driver to run correctly, \n"
                    "while OpenGL version %s, render %s, vendor %s was detected."))) % s_gl_info.get_version_string() % s_gl_info.get_renderer() % s_gl_info.get_vendor()).str());
#endif // ENABLE_OPENGL_ES
            message += "\n";
        ...
        if (valid_version) {
            ...
            if (!result) {
                ...
                wxMessageBox(message, wxString("WaspSlicer - ") + _L("Error loading shaders"), wxOK | wxICON_ERROR);
            }
#if ENABLE_OPENGL_DEBUG_OPTION
