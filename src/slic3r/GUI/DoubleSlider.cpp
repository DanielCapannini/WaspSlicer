...
{ 
    ...
    // See https://github.com/prusa3d/WaspSlicer/issues/6232
...
//    m_ruler.update(this->GetParent(), m_layers_values.empty() ? m_values : m_layers_values, get_scroll_step());
    ...
    if (m_ruler.long_step < 0) {
        ...
        // very short object or some non-trivial ruler with non-regular step (see https://github.com/prusa3d/WaspSlicer/issues/7263)
        else {
...
wxString Control::get_tooltip(int tick/*=-1*/)
{
    ...
    if (tick_code_it != m_ticks.ticks.end())                                    // tick exists
    {
        ...
        auto format_gcode = [space](std::string gcode) {
            // when the tooltip is too long, it starts to flicker, see: https://github.com/prusa3d/WaspSlicer/issues/7368
