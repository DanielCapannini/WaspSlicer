...
# Building WaspSlicer on Mac OS

To build WaspSlicer on Mac OS, you will need the following software:
...
WaspSlicer comes with a set of CMake scripts to build its dependencies, it lives in the `deps` directory.
Open a terminal window and navigate to WaspSlicer sources directory and then to `deps`.
...
If dependencies are built without errors, you can proceed to build WaspSlicer itself.
Go back to top level WaspSlicer sources directory and use these commands:
...
The CMake command above prepares WaspSlicer for building from the command line.
...
and then open the `WaspSlicer.xcodeproj` file.
This should open up XCode where you can perform build using the GUI or perform other tasks.

### Note on Mac OS X SDKs

By default WaspSlicer builds against whichever SDK is the default on the current system.
...
on both the dependencies bundle as well as WaspSlicer itself.

Official macOS WaspSlicer builds are currently (as of WaspSlicer 2.5) built against SDK 10.12 to ensure compatibility with older Macs.
