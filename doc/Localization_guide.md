...
The purpose of this guide is to describe how to contribute to the WaspSlicer translations. We use GNUgettext for extracting string resources from the project and PoEdit for editing translations.
...
Each string resource in WaspSlicer available for translation needs to be explicitly marked using L() macro like this:
```C++
...
2.  Create template file(*.POT) with GNUgettext command:
    ```
        xgettext --keyword=L --add-comments=TRN --from-code=UTF-8 --debug -o WaspSlicer.pot -f list.txt
    ```
...
## General guidelines for WaspSlicer translators
...
- To check how the translated text looks on the UI elements, test it :) If you use *PoEdit*, all you need to do is save the file. At this point, a MO file will be created. Rename it WaspSlicer.mo, and you can run WaspSlicer (see above).

- If you see an encoding error (garbage characters instead of Unicode) somewhere in WaspSlicer, report it. It is likely not a problem of your translation, but a bug in the software.
